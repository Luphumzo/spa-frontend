<template>
    <li class="nav-item nav-search">
        ...
        <div class="search-input" :class="{ open: showSearchBar }">
            ...
            >
                <li
                    v-for="(suggestion_list,
                    grp_name,
                    grp_index) in filteredData"
                    :key="grp_index"
                ...
                >
                    ...
                    <ul>
                        <li
                            ...
                            "
                            @mousedown.prevent="
                                suggestionSelected(grp_name, suggestion)
                            "
                        >
    ...
    setup() {
        ...
        watch(filteredData, val => {
            if (!Object.values(val).some(obj => obj.length)) {
                ...
                for (const [index, grpSuggestions] of Object.values(
                    val
                ).entries()) {
        ...
        const increaseIndex = (val = true) => {
            ...
            if (
                !Object.values(filteredData.value).some(
                    grpItems => grpItems.length
                )
            )
                return;
            ...
            if (val) {
                ...
                if (activeGrpTotalItems - 1 > itemIndex) {
                    currentSelected.value = `${grpIndex}.${Number(itemIndex) +
                        1}`;
                ...
                if (Number(itemIndex)) {
                    ...
                    for (let i = Number(grpIndex) - 1; i >= 0; i--) {
                        // If navigating group have items => Then move in that group
                        if (grpArr[i][1].length > 0) {
                            currentSelected.value = `${i}.${grpArr[i][1]
                                .length - 1}`;
                            break;
                        }
                    }
